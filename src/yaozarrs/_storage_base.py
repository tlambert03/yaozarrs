"""Version-agnostic core of the OME-Zarr storage (hierarchy) validators.

The on-disk rules shared by v0.4, v0.5 and v0.6 (dataset arrays exist and match
the axes, labels groups, plates/wells/fields, bioformats2raw collections) are
implemented once here, in [`BaseStorageValidator`][]. Each version's
`StorageValidatorV0X` subclass (in `yaozarrs.v0X._storage`) sets the
version-specific model classes as class attributes and overrides the small
`_hook` methods below where its spec (or its history) differs.

To make a fix for ONE version only, override the relevant hook in that
version's subclass rather than editing the shared logic here.
"""
...
class LabelsCheckResult:
    ...
    labels_info: tuple[ZarrGroup, Any] | None = None


class BaseStorageValidator:
    """Shared storage-validation logic; see the module docstring.

    Subclasses must set the class attributes below. The defaults of the hook
    methods implement the v0.5 behavior.
    """

    __slots__ = ()

    # --- set by each version's subclass ---------------------------------------
    OME_VERSION: ClassVar[str]
    """Version string used to parse child group metadata (e.g. "0.5")."""
    ROOT_LOC: ClassVar[Loc] = ("ome",)
    """Location prefix for errors at the root document."""
    Image: ClassVar[type[Any]]
    LabelImage: ClassVar[type[Any]]
    LabelsGroup: ClassVar[type[Any]]
    Plate: ClassVar[type[Any]]
    Well: ClassVar[type[Any]]
    Bf2Raw: ClassVar[type[Any]]
    Series: ClassVar[type[Any]]
    OMEAttributes: ClassVar[type[Any]]
    OMEZarrGroupJSON: ClassVar[type[Any]]
    ...
    @classmethod
    def validate_group(
        cls, zarr_group: ZarrGroup, attrs_model: Any | None = None
    ) -> ValidationResult:
        ...
        attrs_model : Any, optional
            The already-validated root metadata (for v0.5+, the `OMEAttributes`
            model; for v0.4 the OME metadata object itself). Parsed from the
            group's attributes if not provided.
        ...
        """
        ome_metadata = cls._root_metadata(zarr_group, attrs_model)
        validator = cls()
        validator._init_root(ome_metadata)
        return validator._dispatch(zarr_group, ome_metadata, cls.ROOT_LOC)

    def _dispatch(
        self, zarr_group: ZarrGroup, ome_metadata: Any, loc_prefix: Loc
    ) -> ValidationResult:
        """Dispatch to the visitor for the type of `ome_metadata`."""
        # NB: LabelImage subclasses Image, so it must be checked first.
        if isinstance(ome_metadata, self.LabelImage):
            return self.visit_label_image(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.Image):
            return self.visit_image(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.LabelsGroup):
            return self.visit_labels_group(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.Plate):
            return self.visit_plate(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.Well):
            return self.visit_well(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.Bf2Raw):
            return self.visit_bioformats2raw(zarr_group, ome_metadata, loc_prefix)
        elif isinstance(ome_metadata, self.Series):  # pragma: no cover
            return self.visit_series(zarr_group, ome_metadata, loc_prefix)
        raise NotImplementedError(
            f"Unknown OME metadata type: {type(ome_metadata).__name__}"
        )

    # ------------------------------------------------------------------------
    # Hooks (override per version)
    # ------------------------------------------------------------------------

    @classmethod
    def _root_metadata(cls, zarr_group: ZarrGroup, given: Any | None) -> Any:
        """Return the root OME metadata object (parsing the group if needed)."""
        attrs_model = given
        if attrs_model is None:
            # extract the model from the zarr attributes
            attrs_model = validate_ome_object(zarr_group.attrs, cls.OMEAttributes)
        return attrs_model.ome

    def _init_root(self, ome_metadata: Any) -> None:
        """Called once with the root metadata, before dispatching."""

    def _parse_attrs(self, attrs: Any) -> Any:
        """Parse a group's attributes into its OME metadata object (or raise)."""
        return validate_ome_object(attrs, self.OMEAttributes).ome

    def _load_source_image(self, uri: str) -> Any:
        """Open `uri` and return its OME metadata object (or raise)."""
        return validate_ome_uri(uri, self.OMEZarrGroupJSON).attributes.ome

    def _child_metadata(
        self, child_group: ZarrGroup, loc: Loc, result: ValidationResult
    ) -> Any:
        """Parse a child group's OME metadata, returning it or the `ValueError`."""
        try:
            return child_group.ome_metadata(version=self.OME_VERSION)
        except ValueError as e:
            return e

    def _visit_multiscale(
        self, zarr_group: ZarrGroup, multiscale: Any, ms_loc: Loc
    ) -> ValidationResult:
        """Validate one multiscale of an image (datasets already prefetched)."""
        return self._visit_multiscale_no_prefetch(zarr_group, multiscale, ms_loc)
    ...
    def _check_dataset_array(
        ...
        """Extra per-dataset-array checks (after existence/ndim checks).

        `previous` holds the (path, array) of the datasets already checked in
        this multiscale.
        """
        self._check_dimension_names(arr, multiscale, ds_loc, result, as_warning=False)
    ...
    def _check_label_path(
        ...
        """Extra checks on a label path that exists (before it is opened)."""

    def _compare_label_to_parent(
        self,
        label_path: str,
        label_loc: Loc,
        label_image_model: Any,
        parent_image_model: Any,
        result: ValidationResult,
    ) -> None:
        """Compare a label image's multiscales against its parent image's."""
    ...
    def _check_well_in_plate(
        self, plate_model: Any, well_model: Any, well_loc: Loc
    ) -> ValidationResult:
        """Extra cross-checks between a plate and one of its (valid) wells."""
    ...
    def visit_label_image(
        self, zarr_group: ZarrGroup, label_image_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
    ...
    def visit_image(
        self, zarr_group: ZarrGroup, image_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        for ms_idx, multiscale in enumerate(image_model.multiscales):
            ...
            result = result.merge(
                self._visit_multiscale(zarr_group, multiscale, ms_loc)
            )
    ...
    def visit_labels_group(
        ...
        labels_model: Any,
        loc_prefix: Loc,
        parent_image_model: Any | None = None,
    ) -> ValidationResult:
        ...
        for label_idx, label_path in enumerate(labels_model.labels):
            ...
            self._check_label_path(labels_group, label_path, label_loc, result)
            ...
            if not isinstance(label_image_model, self.Image):
            ...
            if parent_image_model is not None:
                self._compare_label_to_parent(
                    label_path, label_loc, label_image_model, parent_image_model, result
                )

            if isinstance(label_image_model, self.LabelImage):
    ...
    def _visit_multiscale_no_prefetch(
        self, zarr_group: ZarrGroup, multiscale: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        for ds_idx, dataset in enumerate(multiscale.datasets):
            ...
            self._check_dataset_array(
                zarr_group, multiscale, dataset, arr, ds_loc, result, previous
            )
            previous.append((dataset.path, arr))

        return result

    def visit_plate(
        self, zarr_group: ZarrGroup, plate_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        for well_idx, well in enumerate(plate_model.plate.wells):
            ...
            if isinstance(well_model, self.Well):
                result = result.merge(
                    self._check_well_in_plate(plate_model, well_model, well_loc)
                )
    ...
    def visit_well(
        self, zarr_group: ZarrGroup, well_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        for field_idx, field_image in enumerate(well_model.well.images):
            ...
            if isinstance(field_group_model, self.Image):
                result = result.merge(
    ...
    def visit_bioformats2raw(
        self, zarr_group: ZarrGroup, bf2raw_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        if ome_group is not None and isinstance(ome_group, ZarrGroup):
            try:
                ome_metadata = self._parse_attrs(ome_group.attrs)

                # If OME group has series metadata, use that to find images
                if isinstance(ome_metadata, self.Series):
        ...
        for path in numbered_paths:
            ...
            if isinstance(image_group_meta, self.Image):
                result = result.merge(
    ...
    def visit_series(
        self, zarr_group: ZarrGroup, ome_model: Any, loc_prefix: Loc
    ) -> ValidationResult:
        ...
        for series_idx, series_path in enumerate(ome_model.series):
            ...
            if isinstance(series_group_meta, self.Image):
                result = result.merge(
    ...
    def _check_dimension_names(
        ...
        *,
        ...
        """Check the array's `dimension_names` attribute matches the axes."""
        if dim_names := list(dict(arr.attrs).get("dimension_names", [])):
            ...
            if dim_names != expected_names:
                add = result.add_warning if as_warning else result.add_error
                add(
    ...
    def _add_label_dataset_count_error(
        self,
        label_path: str,
        label_loc: Loc,
        ms_idx: int,
    ...
    def _get_field_paths_from_first_well(
        ...
        try:
            first_well_meta = self._parse_attrs(first_well.attrs)
            if isinstance(first_well_meta, self.Well):
    ...
    def _get_dataset_paths_from_first_field(
        ...
        try:
            first_field_meta = self._parse_attrs(first_field.attrs)
        except Exception:
            return []  # pragma: no cover

        if (
            not isinstance(first_field_meta, self.Image)
            or not first_field_meta.multiscales
        ):
            return []  # pragma: no cover
    ...
    def _check_for_labels_group(
        ...
        try:
            labels_meta = self._parse_attrs(labels_group.attrs)
            if isinstance(labels_meta, self.LabelsGroup):
                # Return the labels info directly
                return LabelsCheckResult(
                    result=result, labels_info=(labels_group, labels_meta)
                )
    ...
    def _validate_labels_image_source(
        ...
        try:
            img_metadata = self._load_source_image(image_source)
            if not isinstance(img_metadata, self.Image):
                result.add_error(
    ...
    def _validate_label_data_types(
        self, image_model: Any, zarr_group: ZarrGroup, loc_prefix: Loc
    ) -> ValidationResult:
                ...
                if isinstance(arr, ZarrArray):
                    ...
                    if not _is_integer_dtype(dt):
                        result.add_error(
                            ...
                            ctx={"path": dataset.path, "dtype": str(dt)},
                        )
